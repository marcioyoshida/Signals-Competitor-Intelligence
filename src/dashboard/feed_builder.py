        ...
        entities.append(
            {
                ...
                # ADR 022 Tier B: monthly balancete trajectory (latest + MoM slope), else None.
                "balancete": balancete_map.get(rec["entity"]),
                # ADR 022 Phase 5: FinBERT financial-tone (inference; corpus pilar3|solvency_facts).
...
def _load_balancete(digests_bucket: str) -> dict[str, dict[str, Any]]:
    """Read the monthly balancete store (ADR 022 Tier B) as {entity_id: trajectory} (latest + MoM),
    best-effort. {} when absent so entities emit balancete=null."""
    try:
        from src.ingest import bcb_balancete

        return bcb_balancete.trajectory_by_entity(bcb_balancete.load_index(digests_bucket))
    except Exception as exc:  # pragma: no cover - best-effort, read-only
        print(f"Warning: load balancete store failed: {exc}")
        return {}


def _load_financial_tone(digests_bucket: str) -> dict[str, dict[str, Any]]:
    """Read the FinBERT financial-tone store (ADR 022 Phase 5) as {entity_id: {net, corpus}},
    best-effort. Labelled inference; corpus='pilar3' (real risk-report text) or 'solvency_facts'."""
    try:
        from src.synth import financial_tone

        idx = financial_tone.load_index(digests_bucket)
        return {eid: {"net": r.get("financial_tone_net"), "corpus": r.get("corpus")}
                for eid, r in (idx.get("records") or {}).items()
                if r.get("financial_tone_net") is not None}
    except Exception as exc:  # pragma: no cover - best-effort, read-only
        print(f"Warning: load financial-tone store failed: {exc}")
        return {}


def _load_fundamentals(digests_bucket: str) -> dict[str, dict[str, Any]]:
    """Read the competitor-fundamentals store (ADR 022 Tier-1) as {entity_id: {roe,roa,...}},
    best-effort. Labelled inference (ROE/ROA annualised)."""
    try:
        from src.ingest import bcb_fundamentals

        return bcb_fundamentals.fundamentals_by_entity(bcb_fundamentals.load_index(digests_bucket))
    except Exception as exc:  # pragma: no cover - best-effort, read-only
        print(f"Warning: load fundamentals store failed: {exc}")
        return {}
...
def _load_resultados(digests_bucket: str) -> dict[str, dict[str, Any]]:
    """Read the operating-efficiency store (ADR 022 Tier-3) as {entity_id: {opex_ativo_pct}},
    best-effort. Annualised opex ÷ ativo from the balancete P&L, labelled inference."""
    try:
        from src.ingest import bcb_resultados

        return bcb_resultados.resultados_by_entity(bcb_resultados.load_index(digests_bucket))
    except Exception as exc:  # pragma: no cover - best-effort, read-only
        print(f"Warning: load resultados store failed: {exc}")
        return {}
