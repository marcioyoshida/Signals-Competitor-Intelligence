"""ADR 006 — Porter's Four Corners: competitor-prediction analysis per entity.

Analyzes each tracked entity across the four corners of competitor behavior
to predict their likely strategic response:

    drivers             — what motivates the competitor (goals, management agenda)
    assumptions         — competitor's beliefs about itself and the industry
    current_strategy    — what the competitor is currently doing
    capabilities        — what the competitor can do (resources, licenses, scale)

A fifth derived dimension, response_profile, synthesizes a predicted next move
from the four corners. This is labeled as inference (never sourced fact) and
rides the ADR 003 predictive axis' citation discipline.

Fed by existing signal: operatives (management/controllers) + CVM IPE fatos +
news/press + the entity's SWOT history. Each entity is analyzed through one
bounded LLM call. Everything is PROPOSED ONLY and flows through Phase-C vetting.

The core `analyze_corners()` is pure — it takes a `draft_fn`, so tests use fakes.
"""
...
FOUR_CORNERS_PROPOSALS_KEY = "four_corners/proposals.json"
...
DIM_LABELS = {
    ...
    "response_profile": "Perfil de resposta provável",
}

ENABLED = os.environ.get("ONCA_FOUR_CORNERS_ENABLED", "1") not in ("0", "false", "False")
MAX_ENTITIES = int(os.environ.get("ONCA_FOUR_CORNERS_MAX_ENTITIES", "8"))
MAX_PER_DIM = int(os.environ.get("ONCA_FOUR_CORNERS_MAX_PER_DIM", "2"))
MIN_CONF = float(os.environ.get("ONCA_FOUR_CORNERS_MIN_CONF", "0.5"))
MIN_EVIDENCE = int(os.environ.get("ONCA_FOUR_CORNERS_MIN_EVIDENCE", "3"))
...
def _bullet_id(entity: str, dim: str, text: str) -> str:
    ...
    return f"four_corners:{entity}:{dim}:{h}"
...
_DRAFT_SYSTEM = (
    "You are a competitive-intelligence analyst performing a Porter's Four Corners "
    "analysis for a Brazilian financial-services competitor, to be reviewed by a human. "
    "The four corners are: drivers (what motivates the competitor — goals, management "
    "agenda, stated ambitions), assumptions (the competitor's beliefs about itself and "
    "the industry — often revealed in IR, press, or regulatory filings), current_strategy "
    "(what the competitor is currently doing — observable moves, product launches, "
    "expansions), capabilities (what the competitor can do — resources, licenses, scale, "
    "technology). Additionally, synthesize a response_profile (predicted likely next move "
    "based on the four corners — this is INFERENCE, not fact). Each assessment must cite "
    "the evidence indices it draws from. Write in pt-BR, one sentence per assessment. "
    "Be specific to this competitor. Return ONLY minified JSON, no prose, no markdown."
)
...
def _draft_prompt(
    ...
    lines += [
        ...
        "Return JSON:",
        '{"corners":[{"corner":"drivers|assumptions|current_strategy|capabilities|response_profile",'
        '"text":"<pt-BR, one sentence assessment>",'
        '"confidence":0..1,"evidence":[<indices into the evidence list>]}]}',
        f"Rules: at most {MAX_PER_DIM} assessments per corner; omit a corner with no "
        "supporting evidence. Each assessment MUST include >=1 valid evidence index. "
        "The response_profile corner is a predicted likely move (inference). "
...
def _parse_draft(raw: str | None, n_evidence: int) -> list[dict[str, Any]]:
    ...
    for item in data.get("corners") or []:
        if not isinstance(item, dict):
            continue
        corner = str(item.get("corner") or "").lower()
        text = str(item.get("text") or "").strip()[:280]
        if corner not in DIMENSIONS or not text:
            continue
        indices = []
        ...
        if not indices:
            continue
        if per_dim.get(corner, 0) >= MAX_PER_DIM:
            continue
        per_dim[corner] = per_dim.get(corner, 0) + 1
        out.append({"corner": corner, "text": text,
...
def analyze_corners(
    ...
    *,
    ...
    """Pure: beliefs + narratives -> Four Corners proposals. No I/O."""
    ...
    for ent in entities:
        ...
        for c in corners:
            ...
            if not ev_ids:
                continue
            proposals.append({
                "id": _bullet_id(ent, c["corner"], c["text"]),
                "kind": "four_corners",
                "framework": FRAMEWORK,
                "entity": ent,
                "label": label,
                ...
                "target_text": None,
                "text": c["text"],
                "is_inference": c["corner"] == "response_profile",
                "evidence": ev_ids,
                "narrative_id": ev_ids[0] if ev_ids else None,
                "date": run_date,
                "confidence": round(c["confidence"], 2),
                "stance_conf": round(c["confidence"], 2),
                "status": "pending",
                "created": run_date,
            })
    return proposals
...
def proposed_entities(bucket: str, *, s3: Any) -> frozenset[str]:
    prev = _load_json(bucket, FOUR_CORNERS_PROPOSALS_KEY, s3).get("proposals", [])
...
def publish(
    ...
    s3.put_object(
        Bucket=bucket, Key=FOUR_CORNERS_PROPOSALS_KEY,
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    window = int(os.environ.get("ONCA_FOUR_CORNERS_WINDOW_DAYS",
    ...
    try:
        ...
        print(f"Warning: Four Corners publish failed: {exc}")
