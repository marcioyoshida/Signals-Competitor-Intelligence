...
| **#80** extend CVM cadastros | **✅ SHIPPED (adm_carteira) + LIVE-VERIFIED** | Added `adm_cart-cad` (Administradores/Gestores de Carteira, CKAN-verified 2026-09-07: **1545 active PJ**, `asset-management`) to `CADASTROS`; PJ csv shares the consultores schema. `_fetch_zip_csv` now reads BOTH a DADOS **zip** and a **direct .csv** resource (CVM serves both). **adm_fii-cad dropped** honestly — its only resource is cancellations-only (135 CANCELADA, no active roster). Securitizadoras have only doc/informe packages (no cadastro). Inherits the default-off `ONCA_INGEST_CVM_PARTICIPANTES` gate (heavy scan). FIDC/CRI/CRA (P4 products) remain doc-package Job-2 work, separate. |
...
| **E2** PREVIC EFPC | **✅ SHIPPED + LIVE-VERIFIED** | `src/ingest/previc_efpc.py` — the PREVIC **Base Cadastral de Entidades** XLSX (verified 2026-09-07: HTTP 200 w/ browser UA at the gov.br `.../base-cadastral-de-entidades.xlsx/@@download/file` direct link — **NOT** behind the dead dados.gov.br token; 447 EFPCs, all with CNPJ; cols Código·Razão Social·Sigla·CNPJ·Situação·UF·Município; title-row offset handled, columns mapped BY HEADER NAME). Stdlib XLSX parse. Gated `ONCA_INGEST_PREVIC`, seed-suppressed, → `new_entrants` → entrants lens + Receita enrich. |
