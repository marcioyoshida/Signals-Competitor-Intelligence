...
def _narratives(entity, n=5):
    return [
        {"id": f"n-{entity}-{i}", "entity": entity,
         "narrative": f"Narrative claim {i} about {entity} market position in the Brazilian sector.",
         "axis": "comparative", "lenses": ["market", "juros"],
...
def _draft_ok(label, industries, swot_bullets, evidence):
    out = []
    if evidence:
        out.append({"quadrant": "star", "text": f"{label} é estrela em pagamentos digitais.",
                    "confidence": 0.85, "evidence": [0]})
        out.append({"quadrant": "cash_cow", "text": "Segmento de cartões gera caixa estável.",
...
def test_classify_portfolio_produces_proposals():
    beliefs = {"acme": _swot_beliefs("acme")}
    narr = _narratives("acme", 5)
    result = bcg.classify_portfolio(beliefs, narr, {},
                                     run_date=RUN, draft_fn=_draft_ok, min_evidence=3)
    assert len(result) >= 2
    for p in result:
        assert p["framework"] == "bcg"
        assert p["kind"] == "bcg"
        assert p["dimension"] in bcg.DIMENSIONS
...
def test_parse_draft_valid():
    raw = '{"positions":[{"quadrant":"star","text":"Estrela.","confidence":0.8,"evidence":[0]}]}'
    result = bcg._parse_draft(raw, 3)
    assert len(result) == 1
    assert result[0]["quadrant"] == "star"


def test_parse_draft_none():
    assert bcg._parse_draft(None, 5) == []


def test_parse_draft_garbage():
    assert bcg._parse_draft("not json", 5) == []


def test_parse_draft_invalid_quadrant():
    raw = '{"positions":[{"quadrant":"invalid","text":"X.","confidence":0.8,"evidence":[0]}]}'
    assert bcg._parse_draft(raw, 3) == []


def test_parse_draft_max_per_dim():
    positions = [{"quadrant": "star", "text": f"Pos {i}.", "confidence": 0.8, "evidence": [0]}
                 for i in range(5)]
    raw = json.dumps({"positions": positions})
    result = bcg._parse_draft(raw, 3)
    assert len(result) == bcg.MAX_PER_DIM


def test_stable_id():
    id1 = bcg._bullet_id("acme", "star", "Some text.")
    id2 = bcg._bullet_id("acme", "star", "Some text.")
    id3 = bcg._bullet_id("acme", "star", "Different.")
    assert id1 == id2
    assert id1 != id3
    assert id1.startswith("bcg:acme:star:")
